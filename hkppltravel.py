...
class HKPplTravel(scrapy.Spider):
    ...
    def parselink(self, response):
        ...
        # these are kind of the subtitles of the activities
        ...
        # this is the second way to get these
        subheader_names =  response.xpath("//strong//text()").getall()

        for subheader_name in subheader_names:
            event['subcategory activities'].append(subheader_name)

        ### there are two formats of names, check for the right formatting
        ...
        ## check for text about the event along with dates, times and locations
        ## dates times and locations are lists because of multiple instances
