...
def convert_date(string):
    ...
    datetime_obj = datetime.strptime(dates[0], full_month_format)
    european_format = "%Y/%m/%d"
    print(f"{datetime.strftime(datetime_obj, european_format)}")
...
class HkGovHad_Activities(scrapy.Spider):
    ...
    def parselink(self, response):
        ...
        for row in rows:
            ...
            event = {'event district': district[:len(district)-1], 'event name': '', 'event start date': [], 'event end date': [],
                     'event duration': [],
            ...
            for string in second_column:
                if is_date(string):
                #    convert_date(string)
                    event['event start date'].append(string)
                    event['event end date'].append(string)
                elif is_time(string):
